CREATE OR REPLACE FUNCTION public.generar_ocurrencias_actividad(p_plantilla_id uuid)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
  ...
  WHILE v_cursor <= v_fin AND v_count < 365 LOOP
    IF EXTRACT(DOW FROM v_cursor)::int = ANY(v_dias) THEN
      INSERT INTO actividades (
        ...
        secciones_objetivo, activo, plantilla_id
      ) VALUES (
        ...
        v_act.secciones_objetivo, v_act.activo, p_plantilla_id
      );
      v_count := v_count + 1;
    END IF;
