...
export function actividadesKey(fecha: Date, residenteId: string | null, seccion: string | null) {
  return ['actividades', toSupabaseDate(fecha), residenteId, seccion];
}
...
export function useActividades(fecha: Date) {
  ...
  const fetchFn = (d: Date) =>
    residenteId
      ? getActividadesPersonalizadas(d, residenteId, miSeccion)
  ...
  useEffect(() => {
    ...
    [-2, -1, 1, 2].forEach((offset) => {
      ...
      queryClient.prefetchQuery({
        queryKey: actividadesKey(d, residenteId, miSeccion),
        queryFn: () => fetchFn(d),
        staleTime: 30 * 60 * 1000,
      });
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fechaKey, residenteId, miSeccion, authLoading]);

  return useQuery({
    queryKey: actividadesKey(fecha, residenteId, miSeccion),
